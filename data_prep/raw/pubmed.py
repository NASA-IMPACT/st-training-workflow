...
_URLs = [
    f"https://ftp.ncbi.nlm.nih.gov/pubmed/baseline/pubmed25n{i:04d}.xml.gz"
...
def deepupdate(target, src):
    ...
    for k, v in src.items():
        ...
        if k in target and type(target[k]) != type(v):
            logger.warning(
                f"Ignoring field {k} it's a {type(v)} and we expect a {type(target[k])}",
            )
            continue
...
class Pubmed(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            name="2025",
            description="The 2025 annual record",
