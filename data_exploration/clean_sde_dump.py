...
def assemble_records_generator(
    input_filename,
    skip_header=False,
        ...
        with open(input_filename, encoding="utf-8") as infile:
            ...
            if skip_header:
                try:
                    ...
                    logger.warning(
                        "Input file appears to be empty or only contains a header.",
                    )
            ...
            while True:
                try:
                    ...
                    is_record_start = (
                        line.startswith("/SDE/")
                        or line.startswith("/SDE-TDAMM/")
                        or line.startswith("/scrapers/")
                    )
                    ...
                    if not record_started and not is_record_start:
                        ...
                        if (
                            skip_header and lines_processed == 2
                        ):  # Only warn on the first actual data line
                            logger.warning(
                    ...
                    logger.warning(
                        f"Skipping line {lines_processed} due to UnicodeDecodeError: {e}",
                    )
...
def process_records_generator(
    raw_records_iterator,
    delimiter="༜",
    ...
    if expected_fields is None:
        raise ValueError(
            "process_records_generator requires 'expected_fields' to be specified.",
        )
    ...
    if invalid_records > 10:
        logger.warning(
            f"Additional {invalid_records - 10} invalid records were skipped (not shown in logs)",
        )
...
def write_output_file(
    ...
    try:
        ...
        logger.info(
            f"Finished writing {records_written:,} records in {total_time:.2f} seconds",
        )
...
def process_large_csv(input_filename, output_filename, delimiter="༜", verbose=False):
    ...
    logger.info(
        f"Processing {input_filename} to {output_filename} with delimiter '{delimiter}'",
    )
    ...
    try:
        with open(input_filename, encoding="utf-8") as infile:
            ...
            if expected_fields == 0:
                logger.error(
                    "Header line is empty or could not be split. Cannot proceed.",
                )
                return -1
    except FileNotFoundError:
        logger.error(f"Input file not found: {input_filename}")
    ...
    try:
        ...
        processed_records_iter = process_records_generator(
            raw_records_iter,
            delimiter,
            expected_fields,
        )
        ...
        records_written = write_output_file(
            output_filename,
            processed_records_iter,
            header,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="Show detailed progress information (INFO level)",
    )
    parser.add_argument(
        "-q",
        "--quiet",
