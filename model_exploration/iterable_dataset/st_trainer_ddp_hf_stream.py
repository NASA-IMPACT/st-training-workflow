...
parser.add_argument("--max_datapoints_per_src_for_eval", type=int, default=-1)
...
def compute_max_steps(n_samples):
    if STREAMING:
        ...
        total_train_samples_one_epoch = sum(
            [nsize.get("train", 0) for src_name, nsize in n_samples.items()],
        )
        effective_global_batch_size_for_update = (
            BATCH_SIZE * world_size * GRADIENT_ACCUMULATION_STEPS
        )
        steps_per_epoch = math.ceil(
            total_train_samples_one_epoch / effective_global_batch_size_for_update,
        )
